...
describe('adresse canonique', () => {
  // Le préfixe d'un hébergement qui sert le site sous le nom du dépôt. Il ne
  // s'applique plus depuis le déplacement — le site est servi à la racine —
  // mais la logique doit rester juste : c'est elle qui avait cédé, et un
  // prochain hébergeur pourrait la réclamer de nouveau.
  ...
  it('n’écrit le préfixe qu’une seule fois', () => {
    ...
    expect(url).toBe('https://anime-ink-blond.vercel.app/Anime-Ink/catalogue')
  ...
  it('préfixe un chemin qui ne l’a pas', () => {
    ...
    expect(urlCanonique('/og-image.svg', BASE))
      .toBe('https://anime-ink-blond.vercel.app/Anime-Ink/og-image.svg')
  })

  it('traite l’accueil comme les autres', () => {
    expect(urlCanonique('/Anime-Ink/', BASE)).toBe('https://anime-ink-blond.vercel.app/Anime-Ink/')
    expect(urlCanonique('/Anime-Ink', BASE)).toBe('https://anime-ink-blond.vercel.app/Anime-Ink')
  })
  ...
  it('ne préfixe rien quand le site est servi à la racine', () => {
    ...
    expect(urlCanonique('/catalogue', '/')).toBe('https://anime-ink-blond.vercel.app/catalogue')
  })
  ...
  it('ne confond pas un chemin qui commence par les mêmes lettres', () => {
    ...
    expect(urlCanonique('/Anime-Inkognito', BASE))
      .toBe('https://anime-ink-blond.vercel.app/Anime-Ink/Anime-Inkognito')
  })
})
